...
const contries = data.categories.Countries;

const unique = contries[Math.ceil(Math.random() * 29)];

const nik = unique.name;
...
const game = createSlice({
  ...
  reducers: {
    ...
    setHeart: (state, { payload }) => {
      state.heart = payload;
    },
    addTOLettter: (state, { payload }) => {
      const one = state.heart.filter(
        (e) => e.let == payload || e.let.toUpperCase() == payload
      );

      if (one.length > 0) {
        one.forEach((e) => state.letters.push(e));
        one.map((e) => (e.selected = true));
        if (state.letters.length == state.heart.length) {
          const modal = document.getElementById("modal");
          state.img = "You Win.png";
          modal.showModal();
        }
      } else {
        const modal = document.getElementById("modal");
        if (state.yurak.length == 1) {
          modal.showModal();
        }

        state.yurak.pop();
        state.img = "You Lose.png";
      }
    },
  },
});

export const { addCountre, setHeart, addTOLettter } = game.actions;
